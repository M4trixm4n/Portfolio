...
static bool SDL_Event_Interface_IaVSIa(SDL_Event_Interface *evt_itf);
    ...
    while (1)
    {
        ...
        while (SDL_PollEvent(evt))
        {
            ...
            case SDL_MOUSEBUTTONUP: // Évenement de pression sur le bouton de la souris
                ...
                if (evt_itf->fenetre->mode_jeu == IAvsIA && stop && isRunning)
                {
...
static int SDL_Event_Interface_event_souris(SDL_Event_Interface *evt_itf)
{
    ...
    case 1:
        if (itf->jeu->mode != IAvsIA)
        {
            ...
            if (!SDL_Interface_actualiserCoup(itf))
                return 2;
        }
        return 0;
    case 2:
        ...
        if (1 == res)
        {
            if (!SDL_Interface_actualiserTexture(itf, itf->paramwindow, evt_itf->choice))
                return 2;
        }
        else if (-1 == res)
        {
...
extern void lancementMVC()
{
    ...
    name[2] = 'L';
    name[3] = '\0';
    interface = startSDL(name, 1020, 1000);
    if (interface != NULL)
    {
        ...
        if (!finJeu(j->b))
        {
            ...
            else if (evt_itf->fenetre->mode_jeu == JvsIA)
            {
                if (evt_itf->joueur == 0 && stop == 0)
                {
                    ...
                    swapTour(board);
                    if (evt_itf->fenetre->jeu->j2->ia != USER)
                        j->j2->play(j);
                    sleep(1);
                    stop = 0;
                }
...
extern bool SDL_Event_Interface_recreerJeu(SDL_Event_Interface *evt_itf)
{
    ...
    if (false == SDL_Interface_creerJeu(itf))
        return false;
    isRunning = false;
    if (itf->mode_jeu == JvsIA && evt_itf->joueur == 1)
    {
        stop = 1;
        sleep(1);
        if (itf->jeu->j2->ia != USER)
    ...
    // SDL_Event_Interface_IaVSIa(evt_itf);
    return true;
}
...
extern void SDL_Event_Interface_setInfoJeu(ModeDeJeu mdj, IA ia1, IA ia2, SDL_Event_Interface *evt_itf, int choice)
{
    ...
    stop = 0;
    if (mdj == IAvsIA || (mdj == JvsIA && evt_itf->joueur == 1))
    {
        stop = 1;
    }
    evt_itf->joueur = 0;
